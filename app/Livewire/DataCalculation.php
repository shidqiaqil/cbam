...
class DataCalculation extends Component
{
    ...
    /** Single-source row - now with HTML bullet */
    private function single(string $label, ?float $val): array
    {
        $fmt = fn(?float $v) => $v !== null ? number_format($v, 3) : '—';
        $html = '<ul class="mb-0"><li>' . str_replace('&#10;', '<br>', $label) . ': <strong>' . $fmt($val) . '</strong></li></ul>';
        return ['value' => $val, 'tooltip' => $html];
    }

    /** Multi-source row: items = [['label', value], ...] - HTML bullets */
    private function multi(array $items): array
    {
        ...
        foreach ($lines as $line) {
            $html .= '<li>' . $line['label'] . ': <strong>' . $line['value'] . '</strong></li>';
        }
        $html .= '</ul>';
        $html .= '<hr class="my-1 opacity-25">';
        $html .= '<div class="text-end fw-bold small">Total: <strong>' . ($hasAny ? number_format($total, 3) : '—') . '</strong></div>';
    ...
    private function adBreakdown7(): array
    {
        ...
        $html = '<ul class="mb-0">';
        $html .= '<li>from master_energy_data<br>plant_code = I01200, plant_name = Purchase for KP<br>criteria = PRODUCTION, energy_name = NG: <strong>' . $fmt($kp) . '</strong></li>';
        $html .= '<li>from master_energy_data<br>plant_code = I01200, plant_name = Purchase for HSM<br>criteria = PRODUCTION, energy_name = NG: <strong>' . $fmt($hsm) . '</strong></li>';
        $html .= '</ul>';
        $html .= '<hr class="my-1 opacity-25">';
        $html .= '<div class="text-end fw-bold small">';
        $html .= 'Raw Sum: <strong>' . ($hasAny ? number_format($raw, 3) : '—') . '</strong><br>';
        $html .= '÷ 1000 = <strong>' . ($hasAny ? number_format($raw / 1000, 3) : '—') . '</strong> (1000Nm³)';
        $html .= '</div>';
