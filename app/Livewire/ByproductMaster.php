...
class ByproductMaster extends Component
{
    use WithPagination;

    #[Url(as: 'tab')]
    public $activeTab = 'byproduct';
    ...
    public function deleteCurrentFilterData()
    {
        ...
        $deletedCount = MasterByproduct::where('period_year', $this->deleteYear)
    ...
    {
        return MasterByproduct::distinct()
    ...
    {
        $byproductData = $this->getByproductDataProperty();
        $years = $this->getYearsProperty();
        $months = $this->getMonthsProperty();

        return view('livewire.byproduct-master', [
    ...
    public function getByproductDataProperty()
    {
        $rawData = MasterByproduct::query();
        ...
        $filtered = collect($pivoted)->filter(function ($row) {
            ...
            return empty($search) ||
                ...
                str_contains(strtolower($row['Source'] ?? ''), $search) ||
                str_contains(strtolower($row['Group'] ?? ''), $search);
        });
